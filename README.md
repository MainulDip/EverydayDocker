...
```
### 3 Tpes of volume:
=> Host Volumes (you define the host directory reference with container directory): -v /home/mount/data:var/lib/mysql/data
=> Anonymous Volumes ( you just reference container dir, for each container that gets mounted a folder is automatically generated by docker): -v /var/lib/mysql/data
=> Named Voules (name with anonymous volumes, so volume can be referenced by name, used in docker-compose mostly): -v name:/var/mysql/data
```sh
